...
Uses DOM inspection, JavaScript object state, and in-browser network capture — no proxy.
...
from core import CheckResult, accept_consent
...
def run_analytics_audit(
    ...
    try:
        with audit_page(capture_network=True) as (page, recorder):
            return _analytics_checks(page, recorder, url, expected_gtm_id, expected_ga4_id)
    except PlaywrightError as exc:
        return [CheckResult(
            name="analytics_audit",
            event=None,
            passed=False,
            detail=f"Browser error during audit: {exc}",
        )]
...
def _analytics_checks(
    ...
    all_resources = (
        _exec(page, "performance.getEntriesByType('resource').map(e => e.name)") or []
    )
